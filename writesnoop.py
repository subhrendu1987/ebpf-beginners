...
#
# writesnoop Trace open() syscalls.
#           For Linux, uses BCC, eBPF. Embedded C.
#
# USAGE: writesnoop [-h] [-T] [-x] [-p PID] [-d DURATION] [-t TID] [-n NAME]
#
# Copyright (c) 2022 Subhrendu Chattopadhyay
#
...
# 15-Jun-2022   Subhrendu       Support for write syscall
...
# arguments
examples = """examples:
    ./writesnoop           # trace all open() syscalls
    ./writesnoop -T        # include timestamps
    ./writesnoop -U        # include UID
    ./writesnoop -x        # only show failed opens
    ./writesnoop -p 181    # only trace PID 181
    ./writesnoop -t 123    # only trace TID 123
    ./writesnoop -u 1000   # only trace UID 1000
    ./writesnoop -d 10     # trace for 10 seconds only
    ./writesnoop -n main   # only print process names containing "main"
    ./writesnoop -e        # show extended fields
    ./writesnoop -f O_WRONLY -f O_RDWR  # only print calls for writing
    ./writesnoop --cgroupmap mappath  # only trace cgroups in this BPF map
    ./writesnoop --mntnsmap mappath   # only trace mount namespaces in the map
"""
parser = argparse.ArgumentParser(
    description="Trace write() syscalls",
...
int syscall__trace_entry_write(struct pt_regs *ctx, const char __user *filename, int flags)
{
"""
...
b = BPF(text='')
# write and openat are always in place since 2.6.16
fnname_write = b.get_syscall_prefix().decode() + 'write'
#fnname_pwrite = b.get_syscall_prefix().decode() + 'pwrite'
#fnname_writev = b.get_syscall_prefix().decode() + 'writev'
#fnname_pwritev = b.get_syscall_prefix().decode() + 'pwritev'
#fnname_pwritev2 = b.get_syscall_prefix().decode() + 'pwritev2'
...
if not is_support_kfunc:
    b.attach_kprobe(event=fnname_open, fn_name="syscall__trace_entry_write")
